...
class RequestManager(models.Manager):
    ...
    def active_users(self, **options):
        if options:
            qs_null = self.filter(user__isnull=False)
            qs_time = timezone.now() - datetime.timedelta(**options)
            qs_gte = qs_null.filter(time__gt=qs_time)
            active = qs_gte.select_related("user").only("user")
            active_set = [request.user for request in active]
            return set(active_set)

# Возвращает список активных пользователей.
# Любые аргументы, переданные этому методу, будут
# переданы timedelta для фильтрации по времени.
# Example:
